...
arch-chroot /mnt reflector --latest 24 --sort rate --save /etc/pacman.d/mirrorlist
...
arch-chroot /mnt ln -sf /usr/share/zoneinfo/Europe/Belgrade /etc/localtime
...
arch-chroot /mnt gpasswd -a $username disk
arch-chroot /mnt gpasswd -a $username storage
arch-chroot /mnt gpasswd -a $username kvm
arch-chroot /mnt gpasswd -a $username input
arch-chroot /mnt echo -e "$root_password\n$root_password" | passwd root
arch-chroot /mnt echo -e "$user_password\n$user_password" | passwd $username


arch-chroot /mnt rm /etc/sudoers
arch-chroot /mnt cp /Athena-Linux/server/files/sysfiles/sudoers /etc/
...
arch-chroot /mnt echo $set_lang > /etc/locale.conf
...
arch-chroot /mnt wget -P /etc/ https://raw.githubusercontent.com/StevenBlack/hosts/master/alternates/fakenews-gambling-porn-social/hosts
...
arch-chroot /mnt rm /etc/pacman.conf
arch-chroot /mnt cp /Athena-Linux/server/files/sysfiles/pacman.conf /etc/
...
arch-chroot /mnt cp /Athena-Linux/server/start_up.sh /home/$username/
arch-chroot /mnt chmod ugo+rwx /home/$username/start_up.sh
...
then
        ...
        arch-chroot /mnt grub-install --target=i386-pc $ssd
        arch-chroot /mnt grub-mkconfig -o /boot/grub/grub.cfg
fi
